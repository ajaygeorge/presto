...
public class ConnectorTypeSerdeManager
{
    ...
    {
        requireNonNull(connectorId, "connectorId is null");
        return Optional.ofNullable(connectorTypeSerdeProviderMap.get(connectorId)).map(ConnectorTypeSerdeProvider::getConnectorMetadataUpdateHandleSerde).orElse(connectorMetadataUpdateHandleJsonSerde);
    }
}
